...
h.addEventListener("input",(e)=>{
   ...
   console.log("e.data",e.data);
    if(/\s/.test(e.data)){
       console.log("h",h.textContent.split(/[\s]+/).slice(-1)[0].replace(/\d{1,3}/,"").trim());
       console.log("aa",aa[cp].split(' ')[0]);
        if((new RegExp(`^${h.textContent.split(/[\s]+/).slice(-1)[0].replace(/\d{1,3}/,"").trim()}$`)).test(aa[cp].split(' ')[0].trim())){
            canDel=false;
